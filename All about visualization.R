...
#Here x and y are plot aesthetic and color is aesthetic for point

# Graph with size element g5
ggplot(diamonds,aes(x=carat,y=price,size=x))+geom_point()
ggplot(diamonds,aes(x=carat,y=price))+geom_point(aes(size=x))
...
#alpha is not in aes as it didn't contain any variable
...
ggplot(diamonds,aes(x=table,y=price))+geom_point(alpha=0.4)+scale_x_continuous(name="Axis for Table",breaks=c(5,10,30,80),limits=c(0,80))
...
#gam- >1000 obs
#lm- linear model     g17
ggplot(diamonds,aes(x=x,y=y))+geom_point(alpha=0.2)+geom_smooth(aes(color=clarity),se=FALSE)
...
# we were using a variable clarity and wanted to color the lines hence we put it inside geom_smooth and not
# inside ggplot
...
#As we mentioned the factor(cyl) inside the ggplot aes so its properties get transfered to geom_point and also
# for geom_smooth where group was not mentioned.


#Overplotting where jittering is required g19 (It adds artificial randomness to the data)
...
#shape and size donot contain variables hence are not in aes
ggplot(diamonds,aes(x=x,y=y,color=color))+geom_point(shape=21,size=4,alpha=0.3)
...
ggplot(mtcars,aes(x=factor(cyl),y=mpg,col=factor(gear),alpha=0.5))+geom_point(position=position_jitter(0.1))

ggplot(mtcars,aes(x=factor(cyl),y=mpg,col=factor(gear),alpha=0.5))+geom_jitter(width=0.1) #same as above



#Faceting facet_grid x~y-  g25  grid means x and y
ggplot(mtcars,aes(x=mpg,y=cyl,col=as.factor(vs)))+geom_point()+facet_grid(am~gear)
...
ggplot(data = mtcars, aes(x = wt, y = mpg)) +
  ...
  geom_line(data = head(mtcars), color = "red")  #aes taken is wt and mpg as there is no change in it
...
#x and only count or probability
?geom_density #which is a smoothed version of the histogram.
...
?geom_freqpoly #Histograms (geom_histogram()) display the counts with bars; frequency polygons (geom_freqpoly()) display the counts with lines.


#x discrete and y either count (bar) or numerical value (col)
?geom_col # its height is as per some continuous y variable
?geom_bar # geom_bar() makes the height of the bar proportional to the number of cases in each group
?geom_area
...
ggplot(mtcars,aes(x=mpg))+geom_density() #y gives the probability of occurance
ggplot(mtcars,aes(x=mpg))+geom_histogram()
ggplot(mtcars,aes(x=mpg))+geom_freqpoly() #same as histogram but shown with just lines

ggplot(mtcars,aes(x=mpg))+geom_freqpoly()+geom_histogram() #here mpg is continuous
ggplot(mtcars,aes(x=mpg))+geom_area(stat="bin") #its same as geom_freqpoly but shade the area underneath
ggplot(mtcars,aes(x=mpg,fill=factor(vs)))+geom_area(stat="bin")


data(mtcars)
#A histogram represents the frequency distribution of continuous variables. Conversely, a bar graph is
#a diagrammatic comparison of discrete variables. Histogram presents numerical data whereas bar graph
#shows categorical data.


ggplot(mtcars,aes(x=factor(cyl)))+geom_bar() #here is cyl is discrete
ggplot(mtcars,aes(x=factor(cyl),y=disp))+geom_col()
...
# By default, the statistical transformation in geom_area is set to identity (direct mapping of variable values from dataset). We change this transformation to count.
...
ggplot(mtcars,aes(x=drat,y=mpg))+geom_col() # it need x and y

ggplot(mtcars,aes(x=drat))+geom_bar()




#Density plot g39
ggplot(mtcars,aes(x=drat))+geom_density(color="black",fill="gray")+geom_vline(aes(xintercept=mean(drat),color="#FC4E07", linetype="dashed"))
...
ggplot(mtcars,aes(x=drat))+geom_density(aes(color=factor(cyl),fill=factor(cyl)))
...
ggplot(mtcars,aes(x=drat))+geom_histogram(aes(y=..density..), colour="black", fill="white") +
  geom_density(alpha=0.2, fill = "#FF6666")  #histogram should go with freqpoly and not density
#frequency ploygon (normal data count) & density plot (normal data %cummulative)



#Histogram with density plot g43
ggplot(mtcars,aes(x=drat))+geom_histogram(aes(y=..density..), colour="red", fill="blue") +
...
ggplot(mtcars,aes(x=drat))+stat_ecdf(geom="point") #g45 How some variable built up
ggplot(mtcars,aes(x=drat))+stat_ecdf(geom="step") #g46


ggplot(mtcars,aes(x=mpg))+stat_ecdf(geom="step")
...
ggplot(mtcars,aes(x=cyl,fill=factor(am)))+geom_bar(position="stack") #g48 (As count)


ggplot(mtcars,aes(x=cyl,fill=factor(am)))+geom_bar(position="fill") #g49 (As proportion)


ggplot(mtcars,aes(x=cyl,fill=factor(am)))+geom_bar(position="dodge") #g50 (count with dodge as per the fill defined)
...
ggplot(mpg, aes(cty, hwy)) +
  geom_point() + geom_quantile() +  #for geom_quantile() we require quantreg
  theme_minimal()
...
f + geom_bar(aes(fill = factor(am)), stat="identity") +
  scale_fill_manual(values = c("#999999", "#E69F00"))  #g67 stat="identity" brings in the characteristics from the ggplot

ggplot(mtcars, aes(x = factor(cyl), y = wt,fill=factor(am)))+geom_col() #using this better than above


f + geom_bar(aes(fill = factor(am)), stat="identity") +
  scale_fill_manual(values = c("#999999", "#E69F00"))  #in bar the


ggplot(mtcars, aes(x = factor(cyl),fill=factor(vs)))+geom_bar(position="stack")+scale_fill_manual(values = c("#999999", "#E69F00"))

ggplot(mtcars, aes(x = factor(cyl),fill=factor(vs)))+geom_bar(position="stack")
...
ggplot(df, aes(x="", y = value, fill=group)) +
  ...
  scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9"))  #same as above


#ggplot(df, aes(x="", y = value, fill=group)) + geom_bar(width = 1, stat = "identity")
#is same as ggplot(df, aes(x="", y = value, fill=group)) + geom_col(width=1)
